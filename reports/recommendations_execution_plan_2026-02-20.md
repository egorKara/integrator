...
- Plan ID: `RECO-EXEC-2026-02-20`
- JSON-источник (SSOT): `reports/recommendations_execution_plan_2026-02-20.json`
...
## 4) Актуальный статус (2026-03-06)
- `PROC-1`: `blocked` — после усиления токена branch protection API возвращает `403 Upgrade to GitHub Pro or make this repository public`.
- `P1-4`: `blocked` — зависит от разблокировки `PROC-1` (внешнее ограничение тарифа/типа репозитория).
- `DOC-1`: `done` — обновлены `README.md` и `docs/CODE_REVIEW.md` с требованиями к токену, путями хранения и процедурой проверки.
- Решение по бюджету: GitHub Pro не рассматривается до появления выручки проекта; актуальный путь разблокировки — подготовка и перевод репозитория в public.
