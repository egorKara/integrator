param(
    ...
    [string]$TaskName = "IntegratorTelegramGithubWorker",
    [string]$LogDir = "",
    [int]$PollSeconds = 90
)
...
$daemonScript = Join-Path $repoRoot "scripts\run_telegram_github_worker_daemon.ps1"

function Test-RequiredEnv {
    $required = @("TELEGRAM_BRIDGE_REPO")
    $missing = @()
...
function New-TaskParts {
    param([string]$Script, [string]$Logs, [int]$Poll)
    $quotedScript = "`"$Script`""
    $argLog = ""
    if ($Logs) {
        $argLog = " -LogDir `"$Logs`""
    ...
    $args = "-NoProfile -ExecutionPolicy Bypass -File $quotedScript$argLog -PollSeconds $Poll"
...
switch ($Action) {
    "install" {
        Test-RequiredEnv
        $parts = New-TaskParts -Script $daemonScript -Logs $LogDir -Poll $PollSeconds
